 ...
 * @author salvador
 */
public class MenuTableroPieza extends javax.swing.JFrame {

    private final Ajedrez ajedrez;

    public MenuTableroPieza(Ajedrez ajedrez) {
    ...
    public boolean selecCasilla(int[] posIni) {
        if (ajedrez.getJuego().esCasillaVacia(posIni)) {
            //JOptionPane.showMessageDialog(null, "La casilla no tiene ninguna pieza");
            return false;
        } else if (!ajedrez.getJugadorAct().getPiezas().contains(ajedrez.getJuego().getPieza(posIni))) {
            //JOptionPane.showMessageDialog(null, "La pieza de la casilla elegida no es tuya");
            //JOptionPane.showMessageDialog(null, ajedrez.getJugadorAct().getPiezas());
            return false;
        }
        return true;
    }
    ...
    private void c00ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c00ActionPerformed
        int[] posIni = new int[]{0, 0};
        if (selecCasilla(posIni)) {
            new MenuTableroMov(ajedrez, posIni).setVisible(true);
            this.dispose();
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(MenuTableroPieza.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
