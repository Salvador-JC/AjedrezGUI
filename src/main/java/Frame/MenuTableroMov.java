 ...
 * @author saljc
 */
public class MenuTableroMov extends javax.swing.JFrame {

    private final Ajedrez ajedrez;
    private final int[] posIni;

    public MenuTableroMov(Ajedrez ajedrez, int[] posIni) {
    ...
    public MenuTableroMov(Ajedrez ajedrez, int[] posFin, boolean necesario) {
        this.ajedrez = ajedrez;
        this.posIni = null;
        initComponents();
    ...
    public boolean selecMov(int[] posFin) {
        if (Arrays.equals(posIni, posFin)) {
            //JOptionPane.showMessageDialog(null, "La pieza ya está en esa casilla");
            return false;
        } else if (ajedrez.getJugadorAct().getPiezas().contains(ajedrez.getJuego().getPieza(posFin))) {
            //JOptionPane.showMessageDialog(null, "La casilla elegida ya está ocupada por una pieza tuya");
            return false;
        } else if (!ajedrez.getJuego().comprobarMovimiento(posIni, posFin, ajedrez.getJuego().getPieza(posIni).getId(), ajedrez.getJugadorAus())) {
            if (!ajedrez.getJuego().comprobarEnroque(posIni, posFin, ajedrez.getJuego().getPieza(posIni), ajedrez.getJugadorAct(), ajedrez.getJugadorAus())) {
                //JOptionPane.showMessageDialog(null, "La pieza no puede moverse a esa casilla");
                return false;
            } else {
                return true;
            }
        } else if (!ajedrez.getJuego().comprobarClavada(ajedrez.getJugadorAct(), ajedrez.getJugadorAus(), ajedrez.getJuego().getPieza(posIni), posIni, posFin)) {
            //JOptionPane.showMessageDialog(null, "La pieza no puede moverse porque esta clavada");
            return false;
        }
        return true;
    }

    private void jaque(int[] posFin) {
        if (ajedrez.getJuego().comprobarJaque(ajedrez.getJugadorAct(), ajedrez.getJugadorAus())) {
            if (ajedrez.getJuego().comprobarMate(ajedrez.getJugadorAct(), ajedrez.getJugadorAus(), posFin)) {
                new MenuJaqueMate(ajedrez).setVisible(true);
                this.dispose();
            } else {
                if (ajedrez.getJugadorAct().isBlanco()) {
                    JOptionPane.showMessageDialog(null, "Jaque de las blancas");
                } else {
                    JOptionPane.showMessageDialog(null, "Jaque de las negras");
                }
    ...
    public void mover(int[] posFin) {
        ajedrez.getJuego().moverPieza(posIni, posFin, ajedrez.getJugadorAus());

        if (ajedrez.getJuego().comprobarCoronar(posFin)) {
            new MenuCoronar(ajedrez, posFin).setVisible(true);
            this.dispose();
        } else {
            jaque(posFin);
        }
    ...
    private void initComponents() {
        ...
        jLabel5.setText("¿A qué casilla quieres mover la pieza?");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(MenuTableroMov.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
